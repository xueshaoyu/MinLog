...
namespace Min.Log
{
    ...
    internal class CompositeLog : Logger
    {
        /// <summary>日志提供者集合</summary>
        public List<ILog> Logs { get; set; } = new List<ILog>();

        /// <summary>日志等级，只输出大于等于该级别的日志，默认Info，打开Uci.LogDebug时默认为最低的Debug</summary>
        ...
        { if (Logs.Contains(log)) Logs.Remove(log); return this; }
