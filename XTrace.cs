...
//20221020增加特殊日志文件处理方法WriteFile(string filename,string info),满足特殊日志文件记录需求
namespace Min.Log
{
    ...
    public static class XTrace
    {
        ...
        private static Dictionary<string, ILog> _SpecialLog = new Dictionary<string, ILog>();

        private static ILog GetSpecialLog(string fileName)
        {
            if (_SpecialLog.ContainsKey(fileName))
            {
                return _SpecialLog[fileName];
            }
            else
            {
                if (InitLog(fileName))
        ...
        /// <summary>输出日志到指定文件，便于特殊日志记录</summary>
        /// <param name="fileName">文件名称</param>
        /// <param name="msg">信息</param>
        public static void WriteFile(string fileName, String msg)
        {
            var log = GetSpecialLog(fileName);
        ...
        static XTrace()
        {
            ...
            //ThreadPool.SetMaxThreads(out int a, out int b);
        }
        ...
        private static Boolean InitLog(string fileName)
        {
            /*
             * 日志初始化可能会触发配置模块，其内部又写日志导致死循环。
             * 1，外部写日志引发初始化
             ...
             * 6，正常写入日志
             */
            var _specialLog = Logger.Null;
            if (_specialLog != null && _specialLog != Logger.Null) return true;
            if (_initing > 0 && _initing == Thread.CurrentThread.ManagedThreadId) return false;
            ...
            lock (_lock)
            {
                ...
                path = Path.Combine(path, "Special");
                path.EnsureDirectory();
                _specialLog = TextFileLog.Create(path, fileName);
            ...
            if (_useConsole)
            {
                var clg = new ConsoleLog { UseColor = _useColor };
                _specialLog = new CompositeLog(clg, _specialLog);
            }

            WriteVersion();
            _SpecialLog.Add(fileName, _specialLog);
            return true;
        }
        ...
        private static Boolean _useColor;
